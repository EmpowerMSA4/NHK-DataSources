...
22/02/2024	1.0.0.1		MSA, Skyline	Initial version
****************************************************************************
*/

// Ignore Spelling: Gqi
namespace GetMultiCastGroups_1
{
	using System;
	...
	public class MyDataSource : IGQIDataSource, IGQIOnInit
	{
		...
		public GQIColumn[] GetColumns()
		{
			return new GQIColumn[]
			{
				...
				new GQIStringColumn("MulticastGroup"),
			};
		}
		...
		public GQIPage GetNextPage(GetNextPageInputArgs args)
		{
			...
			for (int i = leftOff; i < MulticastGroups.Count; i++)
			{
				leftOff++;
				var newRow = new GQIRow(
					new[]
					{
						new GQICell { Value = MulticastGroups.ElementAt(i) },
						new GQICell { Value = MulticastGroups.ElementAt(i) },
					});
				rows.Add(newRow);
			...
			{
				HasNextPage = leftOff < MulticastGroups.Count,
			};
		}
		...
		private void RetrieveMulticastGroups()
		{
			_multicastGroups = new HashSet<string>();
			foreach (var mellanox in Switches)
			{
				foreach (var multicastRoute in mellanox.MulticastRoutes)
				{
					_multicastGroups.Add(multicastRoute.MulticastGroup);
				}
			}
		}
